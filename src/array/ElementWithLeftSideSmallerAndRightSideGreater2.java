...
7
 * */
public class ElementWithLeftSideSmallerAndRightSideGreater2 {
	public static void main(String[] args) {
		try {
			...
			for (int testNo = 0; testNo < noOfTests; testNo++) {
				br.readLine();
				int[] numbers = Arrays.stream(br.readLine().split(" ")).filter(a -> !a.isEmpty())
				...
				for (int i = 0; i < numbers.length; i++) {
					int no = numbers[i];
					if ((i != 0) && (i != numbers.length - 1) && no <= max) {
						invalid = true;
					} else {
						max = no;
						if ((i != 0) && (i != numbers.length - 1) && ((number == -1) || invalid)) {
							number = max;
						}
						invalid = false;
					}
